...
async function get_all_typeoperations({
  ...
  const result = await typeoperation.get_alltypeoperations(
    { page, limit, search, date, typepaiement, devise },
    userconnect.data,
  );
  ...
  try {
    ...
    result.data.forEach((row) => {
      ...
      if (row.ligne_idligneoperation) {
        ...
        const existingLine = op.lignes.find(
          (l) => l.idligneoperation === row.ligne_idligneoperation,
        );
      ...
      if (row.idligneecriture) {
        ...
        if (!ligneExists) {
          ecriture.lignes.push({
            ...
            libelle: row.libelle,
            tauxecriture: row.taux_ecriture,
      ...
      if (row.type_idtypeoperation) {
        const exists = op.caisses.find(
          (t) => t.idtypeoperation === row.type_idtypeoperation,
        );
        if (!exists) {
          const soldeItem = soldes.find(
            (s) => s.idcaisse === row.type_idcaisse,
          );
...
async function create_typeoperation(data) {
  ...
  for (const ligne of data.lignes) {
    ...
    try {
      const ligneoperation = await ligneoperationservice.create_ligneoperation(
        dataligne,
      );
    } catch (error) {
      throw new Error(error);
  ...
  for (const caisse of data.caisses) {
    if (caisse.montantcaisse && Number(caisse.montantcaisse) != 0) {
      ...
      const newtypeoperation1 = new typeoperationmodel(
        ...
        caisse.idperiode,
        societe.data.idsociete ? societe.data.idsociete : null,
        site.data.idsite ? site.data.idsite : null,
        caisse1.idcaisse ? caisse1.idcaisse : null,
      ...
      try {
        recorded1 = await newtypeoperation1.create_typeoperationmodel(
          newtypeoperation1,
        );
      } catch (error) {
        throw new Error(error);
  ...
  if (
    data.demande !== undefined &&
    data.demande !== null &&
    data.demande !== ""
  ) {
    try {
      const decaisse = await demandemodel.decaisse_enteteDemande(
        data.demande,
        1,
      );
    } catch (error) {
...
async function get_by_idtypeoperation(idtypeoperation) {
  ...
  try {
    const typeoperation_ = await typeoperation.get_onetypeoperation(
      idtypeoperation,
    );
...
async function update_typeoperation(idtypeoperation, data) {
  ...
  for (const ligne of data.lignes) {
    ...
    try {
      const ligneoperation = await ligneoperationservice.update_ligneoperation(
        ligne.idligne,
        dataligne,
      );
    } catch (error) {
  ...
  for (const caisse of data.caisses) {
    if (caisse.montantcaisse && Number(caisse.montantcaisse) != 0) {
      ...
      let recorded1 = null;
      try {
        recorded1 = await newtypeoperation1.update_typeoperation(
          caisse.idtypeoperation,
          newtypeoperation1,
        );
      } catch (error) {
  ...
  try {
    let enteteoperation = null;
    enteteoperation = await enteteoperationservice.get_by_identeteoperation(
      data.idoperation,
    );
    return enteteoperation;
  } catch (err) {
...
async function delete_typeoperation(idtypeoperation) {
  try {
    const typeoperation_ = await typeoperation.delete_typeoperation(
      idtypeoperation,
    );
...
async function getDataRecu_operation(idoperation) {
  if (!idoperation) {
    throw new Error("Erreur de donnée");
  }
...
async function cancel_enteteoperation(data) {
  ...
  try {
    ...
    await enteteoperationservice.update_status(data.idoperation, {
      annulee: 1,
 ...
 * @param {string} numero - Code de l'opération (ex: "OPE-2026-001")
 * @returns {Promise<object>} - Données formatées pour le reçu
 */
async function getRecuDataByNumero(numero) {
  if (!numero) {
    throw new Error("Le numéro du reçu est requis.");
  }

  // 1. Récupère l'ID de l'opération via son code
  const operation = await typeoperation.get_by_codeoperation(numero);
  if (!operation?.idoperation) {
    throw new Error("Reçu non trouvé.");
  }

  // 2. Récupère les données détaillées du reçu
  const recuData = await getDataRecu(operation.idoperation);
  return {
    ...recuData,
    idoperation: operation.idoperation, // Optionnel: ajoute l'ID si besoin
    codeoperation: operation.codeoperation,
  };
}

async function getDataDocumentJustificatif(idoperation) {
  if (!idoperation) {
    throw new Error("Erreur : idoperation manquant");
  }
  ...
  try {
    ...
    const head = operationRows[0];

    // =========================
    // 1. SYNTHÈSE OPÉRATION
    // =========================
    const synthese = {
      montantDecaissement: head.montant_decaissement,
      montantDecaissementRef: head.montant_decaissement_ref,
      montantJustifie: head.montant_justifie,
      montantJustifieRef: head.montant_justifie_ref,
    ...
    detailsJustifRows.forEach(r => {
      ...
      justifMap[r.idjustificatifoperation].details.push({
        id: r.iddetailsjustificatifoperation,
        nature: r.natureoperation,
        centre: r.centreanalytique,
        codecentre: r.codecentreanalytique,
    ...
    const justificatifs = Object.values(justifMap);
    ...
    console.error(`Erreur service justification: ${err}`.red);
    throw err;
  }
}
