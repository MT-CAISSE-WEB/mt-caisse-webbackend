  ...
  async update_enteteoperation(codeoperation, data) {
    ...
    try {
      const check = await pool
        ...
        .query(
          `SELECT COUNT(*) AS count FROM EnteteOperationCaisse WHERE codeoperation = @codeoperation`,
        );
      ...
      if (check.recordset[0].count > 0) {
        ...
        const prefix = "num";
        const numerogenere = await this.create_numoperation(
          prefix,
          data.dateoperation,
        );
  ...
  async delete_enteteoperation(idoperation) {
    ...
    try {
      const result = await pool
        ...
        .input("idoperation", sql.UniqueIdentifier, idoperation)
        .query(
          "DELETE FROM EnteteOperationCaisse WHERE idoperation = @idoperation",
        );
    ...
    try {
      const result = await pool
        ...
        .input(
          ...
          sql.UniqueIdentifier,
          statusData.idoperationannulation,
        )
        .input("updatedat", sql.DateTime, statusData.updatedat)
        .input("updatedby", sql.NVarChar(100), statusData.updatedby)
        .query(
          "UPDATE EnteteOperationCaisse SET annulee = @annulee, idoperationannulation = @idoperationannulation, updatedat = @updatedat, updatedby = @updatedby WHERE idoperation = @idoperation",
        );
      return { success: true, data: result };
    } catch (error) {
  ...
  async update_operationorigine(idoperation, data) {
    ...
    try {
      const result = await pool
        ...
        .query(
          "UPDATE EnteteOperationCaisse SET idoperationorigine = @idoperationorigine, updatedat = @updatedat, updatedby = @updatedby WHERE idoperation = @idoperation",
        );
      return { success: true, data: result };
    } catch (error) {
      console.log(`Erreur de modification du statut: ${error}`.cyan.bold);
