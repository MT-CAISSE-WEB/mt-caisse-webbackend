...
module.exports.getallsocietes = asyncHandler(async (req, res, next) => {
  try {
    ...
    res.status(societes.status).json({
      success: societes.success,
      message: societes.message,
      data: societes.data,
    });
  } catch (error) {
...
module.exports.getonesociete = asyncHandler(async (req, res, next) => {
  try {
    ...
    res.status(societe.status).json({
...
module.exports.upsertsociete = asyncHandler(async (req, res, next) => {
  try {
    ...
    const updatedsociete = await societeservice.upsertsociete({
      codesociete,
      iddevisereference,
      iddevisereporting,
      raisonsociale,
    ...
    res.status(updatedsociete.status).json({
      success: updatedsociete.success,
      message: updatedsociete.message,
      data: updatedsociete.data,
    });
  } catch (error) {
    res.status(500).json({ success: false, message: "Erreur serveur", error });
  }
...
module.exports.deletesociete = asyncHandler(async (req, res, next) => {
  try {
    ...
    res
      .status(deletesociete.status)
      .json({ success: deletesociete.success, message: deletesociete.message });
  } catch (error) {
    res.status(500).json({ success: false, message: "Erreur serveur", error });
  }
});
